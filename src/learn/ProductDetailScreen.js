  ...
  const ProductDetailScreen = ({navigation, route}) => {
    ...
    const showCartQuantity = async () => {
      try {
        ...
        // Check if responseData is not empty and if entries exist
        if (responseData.cart && responseData.cart.entries && responseData.cart.entries.length > 0) {
          setCartQuantity(responseData.cart.entries[0].quantity);
        } else {
          setCartQuantity(0); // Or any other default value you prefer if cart is empty
        }
      } catch (error) {
